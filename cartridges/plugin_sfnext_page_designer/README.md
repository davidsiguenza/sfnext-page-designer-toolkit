...
`plugin_sfnext_page_designer` is a reusable Page Designer toolkit for Salesforce B2C Commerce and Storefront Next. It provides 9 merchant-facing page types, 37 component types, and 6 custom editors without brand assets, catalog IDs, credentials, or fixed site configuration. The optional Size Guide intentionally includes a versioned Mayoral sizing dataset; no Mayoral imagery or product data is bundled.
...
| `component.SFNextToolkit.countdownBand` | Manual or campaign countdowns with optional CTA and native On Date preview. | Optional campaign API |
...
Countdown Band provides independent start/end counters, manual date/time/timezone editors, a native campaign selector, styling, and an optional CTA. It uses the native Business Manager `__siteDate` preview value in the editor browser’s timezone and applies the selected campaign phase even in `mode=EDIT`.

The cartridge includes three additional editors and the `pd-countdown` Custom API. Campaign mode requires the `c_pdcountdown` scope on the target SLAS client and the generated `pdCountdown` client in Storefront Next. See the [full component guide](../../src/components/sfnext-toolkit/countdown-band/README.md) for setup, preview checks, troubleshooting, and maintenance.

### Shoppable Image

Use Shoppable Image for campaign photography where shoppers should reach every featured product without searching. Merchants can freely select up to 12 catalog products or choose a Product Set and validate every hotspot against its members. Coordinates are stored as percentages; desktop coordinates remain stable as the uncropped image scales, while an optional mobile crop can use independent coordinates.

Each 44 px hotspot is a semantic button. Pointer activation, Enter, or Space opens the standard storefront Quick Add, including variant and size selection. Desktop hover and keyboard focus can reveal the current catalog name, market price, and availability. The optional panel visually attaches the heading, description, view-all modal, and Product Set actions to the image; merchants can hide only the view-all CTA or the entire panel.

The visual editor cannot read sibling Page Designer image fields from its isolated iframe. Choosing the same file inside the Studio creates only a temporary browser preview; it is never saved, uploaded again, or used by the storefront. The native image attribute remains authoritative and selects media from the current B2C Commerce site library. Every hotspot and Product Set uses Page Designer's native Product Picker.

Salesforce CMS content is a separate Page Designer source (`cms_record`), not an option inside the native B2C image picker. Supporting it would require a dedicated CMS content type and an explicit component source mode; the hotspot editor still could not read that sibling CMS attribute automatically.

Live product details are loaded through SCAPI with the active site, locale, currency, price books, and inventory. Missing or market-unassigned products are omitted live and diagnosed in authoring mode instead of leaving dead hotspots. See [`src/components/sfnext-toolkit/shoppable-image/README.md`](../../src/components/sfnext-toolkit/shoppable-image/README.md) for the complete authoring and integration contract.
...
When including Countdown Band, also merge its generated SCAPI client or regenerate it from the API schema, retain the plugin’s `rest-apis/pd-countdown` files, and add `c_pdcountdown` to the SLAS client for campaign mode. Follow the [component installation steps](../../src/components/sfnext-toolkit/countdown-band/README.md#install-in-another-project).
...
7. Run `pnpm cartridge:generate` and `pnpm cartridge:validate`. Do not copy a stale generated registry or edit it manually. Validation should report 52 JSON toolkit metadata files (37 components, 9 pages, and 6 custom editors), plus the editors' server/static resources.
