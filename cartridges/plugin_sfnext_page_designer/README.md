...
The visual editor cannot read sibling Page Designer image fields from its isolated iframe. Choosing the same file inside the Studio creates only a temporary browser preview; it is never saved, uploaded again, or used by the storefront. The native image attribute remains authoritative. The native Product Picker is always available. Optional inline name/ID search requires deployment of `src/routes/resource.shoppable-product-search.ts` and an absolute HTTPS `productSearchEndpoint` in the component metadata. Leave that value empty in a portable build until the destination MRT host is known.
