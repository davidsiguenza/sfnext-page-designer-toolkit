                ...
                if (Node.isArrayLiteralExpression(firstArg)) {
                    ...
                    for (const element of elements) {
                        if (Node.isObjectLiteralExpression(element)) {
                            ...
                            // Region descriptions are useful to the Storefront Next design runtime,
                            // but deployed B2C 26.7 schemas reject them as additional properties.
                            // Emitting one invalidates the complete page/component type, so retain
                            // the authored value in decorator metadata only.
