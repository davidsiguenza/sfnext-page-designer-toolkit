...
type SimpleAttributeValue = string | number | boolean;
type ProductSearchHitVariant = NonNullable<ShopperSearch.schemas['ProductSearchHit']['variants']>[number];

const isSimpleAttributeValue = (value: unknown): value is SimpleAttributeValue =>
    typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean';

const formatAdditionalAttributeValue = (value: unknown): string | null => {
    if (isSimpleAttributeValue(value)) {
        const formattedValue = String(value);
        return formattedValue.trim() ? formattedValue : null;
    }

    if (Array.isArray(value) && value.every(isSimpleAttributeValue)) {
...
const getAdditionalAttributeValue = (
    ...
    // Search hits may carry custom values at any of these levels depending on
...
export interface ProductTileProps extends ComponentProps<'div'> {
    ...
    /** Product-list-level presentation authored in Page Designer. */
        ...
        (
            {
                ...
                maxSwatches: maxSwatchesProp = PRODUCT_TILE_MAX_SWATCHES,
            ...
            const presentation = useMemo(
                () => ({ ...DEFAULT_PRODUCT_LIST_CONFIG, ...tilePresentation }),
                [tilePresentation]
            );
            const configuredMaxSwatches = tilePresentation?.maxSwatches ?? maxSwatchesProp;
            const maxSwatches = Number.isFinite(configuredMaxSwatches)
                ? Math.max(0, Math.trunc(configuredMaxSwatches))
                : DEFAULT_PRODUCT_LIST_CONFIG.maxSwatches;

            const additionalAttributeValues = useMemo(
                () =>
                    product
                        ? presentation.additionalAttributes.flatMap(({ id, label }) => {
                              const value = getAdditionalAttributeValue(product, representedVariant, id);
                              return value === null ? [] : [{ id, label: label || id, value }];
                          })
                        : [],
                [presentation.additionalAttributes, product, representedVariant]
            );
            ...
            return (
                <Card
                    ...
                    <div className="product-image relative">
                        <div className="relative w-full overflow-hidden">
                            <ProductImageContainer
                                product={product}
                                imageViewType={presentation.imageViewType}
                                selectedColorValue={
                            ...
                            {/* Badges — top-left */}
                            {presentation.showBadges && productData?.badges.hasBadges && (
                            ...
                            {(showPickupAvailable || presentation.showWishlist) && (
                    ...
                    <div className="relative p-4">
                        {/* Color swatches */}
                        {presentation.showSwatches && colorValues.length > 0 && (
                            <div>
                        ...
                        {presentation.showBrand && (
                            <p className="text-sm font-normal leading-none text-muted-foreground mb-1">
                                {(product as { brand?: string }).brand ?? config.global.branding.name}
                            </p>
                        )}

                        {/* Top category */}
                        {presentation.showCategory && topCategoryName && (
                            <p className="text-sm font-normal leading-none text-muted-foreground mb-1">
                                {topCategoryName}
                            </p>
                        )}
                        ...
                        <h3
                            className={
                                presentation.showProductName
                                    ? 'text-lg font-semibold leading-[120%] tracking-[-0.45px] text-card-foreground mb-2'
                                    : 'sr-only'
                            }>
                            <Link
                                to={productUrl}
                        ...
                        {/* SKU */}
                        {presentation.showSku && product.productId && (
                            <p
                        ...
                        {additionalAttributeValues.length > 0 && (
                            <dl className="mb-2 space-y-1" data-testid="product-tile-additional-attributes">
                                {additionalAttributeValues.map(({ id, label, value }) => (
                                    <div key={id} className="flex gap-1 text-sm text-muted-foreground">
                            ...
                            <>
                                <div>
                                    <ProductPrice
                                        ...
                                        className="text-sm"
                                        hidePromo={!presentation.showPromotions}
                                    />
                                </div>
