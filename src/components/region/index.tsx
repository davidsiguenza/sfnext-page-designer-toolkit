...
export function Region(props: RegionProps) {
    ...
    if (props.component !== undefined) {
        ...
        const resolvedRegion =
            props.component.regions?.find((region) => region.id === regionId) ||
            (isDesignMode && metadata
                ? ({ id: regionId, components: [] } as ShopperExperience.schemas['Region'])
                : undefined);

        if (!resolvedRegion || (fallbackOnEmpty && !resolvedRegion.components?.length)) {
            return errorElement ?? null;
        }

        return renderRegionContent(resolvedRegion, regionId, metadata, className, rest, errorElement, isDesignMode);
    }
