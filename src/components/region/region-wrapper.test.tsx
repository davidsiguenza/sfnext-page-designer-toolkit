...
describe('RegionWrapper', () => {
    ...
    test('(runtime) preserves data and accessibility props on the region wrapper', () => {
        vi.mocked(usePageDesignerMode).mockReturnValue({ isDesignMode: false, isPreviewMode: false });
        const region = makeRegion('r1', []);

        const { container } = render(
            <RegionWrapper region={region} data-slot="nested-items" aria-label="Nested items">
                <div data-testid="kid" />
            </RegionWrapper>
        );

        const wrapper = container.querySelector('[data-slot="nested-items"]');
        expect(wrapper).toHaveAttribute('aria-label', 'Nested items');
        expect(wrapper).toHaveAttribute('data-slot', 'nested-items');
        expect(wrapper).toContainElement(screen.getByTestId('kid'));
    });
