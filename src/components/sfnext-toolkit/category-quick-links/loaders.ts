...
/** Loads an alternate quick-link source category; the route category is used when this returns null. */
export async function loader({
    ...
    const sourceCategory = (component.data as { sourceCategory?: unknown } | undefined)?.sourceCategory;

    if (typeof sourceCategory !== 'string' || !sourceCategory.trim()) {
        return null;
    }

    return fetchCategory(context, sourceCategory.trim(), 1);
}
