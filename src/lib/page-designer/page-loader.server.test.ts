...
import {
    ...
    fetchPageWithComponentDataOrThrow,
} from './page-loader.server';
...
describe('pageLoader', () => {
    ...
    describe('fetchPageWithComponentDataOrThrow', () => {
        test('attaches component loader promises to the resolved page', async () => {
            mockedRegistry.hasLoaders.mockReturnValue(true);
            mockedRegistry.callLoader.mockReturnValue(Promise.resolve({ title: 'Loaded' }));
            mockedFetchPage.mockResolvedValue(
                createMockPage([createMockRegion([createMockComponent('hero-1', 'Content.hero')])])
            );

            const result = await fetchPageWithComponentDataOrThrow(createLoaderArgs(BASE_URL), {
                pageId: MOCK_PAGE_ID,
            });

            await expect(result.componentData?.['hero-1']).resolves.toEqual({ title: 'Loaded' });
        });

        test('preserves API errors for primary page routes', async () => {
            const serverError = new ApiError({
                status: 500,
                statusText: 'Internal Server Error',
                headers: new Headers(),
            ...
            await expect(
                fetchPageWithComponentDataOrThrow(createLoaderArgs(BASE_URL), { pageId: MOCK_PAGE_ID })
