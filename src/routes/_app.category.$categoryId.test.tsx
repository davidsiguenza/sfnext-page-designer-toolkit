...
vi.mock('@/components/region', () => ({
    Region: ({ errorElement }: any) => errorElement ?? null,
}));
...
describe('CategoryPage', () => {
    ...
    describe('Decorators', () => {
        ...
        test('should have RegionDefinition decorator with configurable product-list region', () => {
            ...
            expect(productListRegion?.name).toBe('Configurable Product List');
            expect(productListRegion?.maxComponents).toBe(1);
            expect(productListRegion?.componentTypeInclusions).toEqual(['Layout.productList']);
    ...
    describe('loader', () => {
        ...
        test('should derive search image and custom-variation fields from the Page Designer product list', async () => {
            (fetchPageWithComponentData as any).mockResolvedValue({
                ...createMockPage([
                    {
                        id: 'plpProductList',
            ...
            for (const [, parameters] of (fetchSearchProducts as any).mock.calls) {
                expect(parameters).toEqual(
                    expect.objectContaining({
                        imgTypes: 'hi-res',
                        includedCustomVariationProperties: ['c_material', 'c_fit'],
                    })
                );
            }
